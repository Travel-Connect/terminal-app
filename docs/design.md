...
- セッションは `session_id` 単位で保持し、タイルは**最終イベント時刻が最新のセッション**の状態を表示する。
...
| 発光 | **キーの下から漏れる光**。点滅ではなく約 **2.4 秒周期の「呼吸」**（CSS 例: `animation: breath 2.4s ease-in-out infinite`）。発光の強さ/速さの増強は OPEN-05 |
| 状態色 | 完了 = 緑（呼吸発光）／確認待ち = アンバー（呼吸発光。緑と並んでも見分けられる二系統）／エラー = **動かない赤**（アニメーションなし）／実行中 = ニュートラル＋スピナー／待機 = ニュートラル無発光（モック未記載・設計追加、5.1 参照） |
| 表示要素 | プロジェクト名／状態アイコン（✓ ? ⚠ スピナー）／状態ラベル＋相対時刻（「完了・2分前」）または経過時間（実行中 1:24:01） |
