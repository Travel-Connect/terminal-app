...
| 表示要素 | 1 行目: プロジェクト名（分割タイルは ①② の番号付き。260904_1 #3）／2 行目: 手動ステータスのバッジ（260727_1。260904_1 #1 で名前の横から下の行へ移動 — 名前と行を取り合わない）／状態アイコン（✓ ? ⚠ スピナー）＋作業テキスト／状態ラベル＋相対時刻（「完了・2分前」）または経過時間（実行中 1:24:01） |
