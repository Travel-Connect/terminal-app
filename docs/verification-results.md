...
## 9. Codex の状態タイル（2026-09-25）

- `npm test`: 75 ファイル / 647 件が通過。`npm run typecheck`、`npm run lint`、`npm run build` が成功。
- `node scripts/verify-codex-e2e.mjs`: 17 項目が通過。証跡は `docs/evidence/20260925-codex/`。
  使い捨て DB・専用 dataDir・擬似 Claude 登録簿を使用し、外部 API は無効化。
  Windows の実保存形式 `\\?\` の cwd で、Codex の実行中→完了→次のターン→返答待ち、
  Claude との分割表示、Claude 掃引 2 回後の表示維持、再起動後の復元を確認した。
- 単体テストでは DB が読み取り前後で不変であること、子・自動実行・古い履歴の除外、
  未知の状態・壊れたデータ、構造化質問への回答、表示クリア後の古い完了履歴の復活防止を確認した。
- 本機の読み取り確認では登録済みプロジェクトに 6 タイルを対応付け、今回の terminal-app セッションを実行中として検出。
  約 30ms で取得した。保存済み設定のバックアップを作成して `monitorCodex: true` を設定し、
  稼働アプリを通常終了・再起動。ログの「Codex 監視: ローカル履歴に接続」とウィンドウ表示を確認した。
- 制約: 権限承認待ちは検知しない。30 分無更新で切断扱い、対象は直近 24 時間の対話。
  Codex のローカル DB 形式変更時は追従が必要。実際の会話・認証情報は証跡に含めていない。
