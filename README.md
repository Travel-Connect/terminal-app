...
- **タイル名の見直し提案（260922_6）**: Jev が「表示名がその作業を表しているか」「表示名が汎用的すぎないか」を判定し、見直した方がよいタイルには名前の下に黄色の印（例「✎ 名前が作業を表していません」）が出る。判定は表示名か作業テキストが変わったときだけ（同じ材料では聞き直さない）。実測では `dev` のような汎用名が 0.98、具体名は 0.08〜0.41 で分かれた。
  タイル右クリック →「表示名を AI に提案（Claude Sonnet）」で、フォルダ名と直近の作業から新しい表示名を 1 つ作り、確認ダイアログで採用するとその場で改名する（Jev は文章を作れないため、名前の生成だけ Claude Code CLI の sonnet に任せる）。`claude` コマンドが無い・時間切れ・形式が想定外なら何も変えずステータスバーに理由を出す。
