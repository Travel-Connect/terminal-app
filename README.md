...
- **Jev 判定（260922_2）**: TypeSafe AI の判断専用モデル Jev を「制御層の門番」として使い、次の 4 つを補う。(1) **返答待ち** — Claude が文章で質問して止まった Stop は「完了」でなく「返答待ち」（確認待ちと同じ青の点滅・左上・トースト「返答が必要です」）。(2) **確認待ちの危険度** — 権限確認のツール呼び出しが「取り消せない操作」「外部へ送る操作」「広範囲に影響」なら名前の下に赤い印。(3) **作業テキストの上書き防止** — 「はい」「A」「続けて」のような短い返事では作業テキストを前の依頼文のまま残す。(4) **停滞の疑い** — 実行中のタイルで同じ失敗を繰り返している・進展が無いと判定されると赤い印「停滞の疑い・…」。
  API キーは env `TYPESAFE_API_KEY` か `%USERPROFILE%\.typesafe.env` の `TYPESAFE_API_KEY=...`（console.typesafe.ai で発行）。無ければ・env `TERMINAL_APP_JEV=off` なら・API が失敗したら、これらの印が出ないだけで従来どおり動く。判定結果は `app.log` に「Jev 〜判定:」で全件残る。transcript の抜粋（最後の返答・許可待ちのツール呼び出し・直近の手順）が米国の外部 API へ送られる点に注意（顧客データを扱うプロジェクトでは `TERMINAL_APP_JEV=off` で止める）。実 API の疎通結果: `docs/evidence/20260922-jev/probe-result.json`。
